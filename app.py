...
window = Tk()
window.title("Solicitous Screen Recorder")
window.geometry('350x200')
window.configure(bg='white')

lbl = Label(window, text="Welcome to Py Screen Recorder",
            font=("Arial Bold", 12), bg='white')
...
def record():
    ...
    out = cv2.VideoWriter(filename, fourcc, 0.0, (SCREEN_SIZE))
    global r
    while True:
...
btn = Button(window, text="Start Recorder", command=clicked,
             bg='#eeeeee', borderwidth=0, foreground='#000000')
btn.place(bordermode=OUTSIDE, x=20, y=60, height=50, width=310)

btnexit = Button(window, text="Exit", command=exitapp,
                 bg='#ee0000', borderwidth=0, foreground='#ffffff')
